...
public class ChatClientFrame extends JFrame {
	...
	private JScrollPane West_ScrollPane_1;
	...
	public ChatClientFrame() throws Exception {
		...
		West_ScrollPane_1 = new JScrollPane();
		contentPane.add(West_ScrollPane_1, BorderLayout.WEST);
		
		chatList = new JList();
		chatList.setModel(new AbstractListModel() {
			String[] values = new String[] {"박주용", "김땡땡", "최땡땡", "이땡땡"};
			public int getSize() {
		...
		West_ScrollPane_1.setViewportView(chatList);
		...
		public void run() {
			...
			while(true) {
				...
				if(chatArray[0].equals(ChatProtocol.PLAIN_MSG)) {
					...
					System.out.println("B.ClientClientThread:서버로부터 읽은 데이타로 클라이언드 LIST갱신.");
